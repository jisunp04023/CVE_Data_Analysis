...
data = pd.read_json("./input/cve_list.txt")
...
NodeTable.to_csv("./data/Node.csv", index = False)
...
# .xlsx 파일 생성
with pd.ExcelWriter('./results/DataAnalysis.xlsx') as writer:
...
for i in range(len(Sample)):
    for j in range(i+1, len(Sample)):
        ...
        if(float(s) >= threshold):
            weight.append([])
            weight[idx2].append(ManageLabel(i)) # Source
            weight[idx2].append(ManageLabel(j)) # Target
            weight[idx2].append(s)              # Weight
            idx2 += 1
            
weight = pd.DataFrame(weight)
weight.columns = ['Source', 'Target', 'Weight']

weight.to_csv("./data/Edge.csv", sep = ' ', index = False)

Label = []
for i in range(len(LabelList)):
    Label.append(node['Label'].loc[LabelList[i]])
Label = pd.DataFrame(Label)
Label.columns = ['Label']

Label.to_csv("./data/Label.csv", sep = ' ', index = False)

with open("./data/Edge.csv", 'r') as f:
    with open("./data/CVE_Edge.csv", 'w') as f1:
        next(f)
        for line in f:
            f1.write(line)